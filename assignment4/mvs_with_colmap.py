...
def run_colmap_with_fallback(command_variants):
    """Run the first COLMAP command variant that is accepted by this version."""
    last_error = None
    for command in command_variants:
        try:
            ...
            last_error = error
            print("COLMAP command failed, trying fallback if available:")
            print(" ".join(command))
    raise last_error
...
if __name__ == '__main__':
    ...
    feature_base = ['colmap', 'feature_extractor', '--image_path', os.path.join(data_dir, 'images'), '--database_path', os.path.join(data_dir, 'database.db'), '--ImageReader.single_camera', '1', '--ImageReader.camera_model', 'PINHOLE']
    run_colmap_with_fallback([
        feature_base + ['--FeatureExtraction.use_gpu', '0'],
        feature_base + ['--SiftExtraction.use_gpu', '0'],
        feature_base,
    ])

    # Feature matching
    matcher_base = ['colmap', 'exhaustive_matcher', '--database_path', os.path.join(data_dir, 'database.db')]
    run_colmap_with_fallback([
        matcher_base + ['--FeatureMatching.use_gpu', '0'],
        matcher_base + ['--SiftMatching.use_gpu', '0'],
        matcher_base,
    ])

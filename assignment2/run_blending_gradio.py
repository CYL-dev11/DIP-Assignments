...
def add_point(img_original, polygon_state, evt: gr.SelectData):
    ...
    # 动态计算线条宽度和点的大小，防止在大图上看不见
    w, h = img_with_poly.size
    line_width = max(int(min(w, h) * 0.005), 2)
    point_radius = line_width + 2
...
def create_mask_from_points(points, img_h, img_w):
    ...
    # 2. 将 numpy 数组格式的点转换为 PIL 需要的元组列表格式
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    """
    计算前景图和混合图在各自掩码区域内的拉普拉斯损失。
    """
    device = foreground_img.device
    
    # 1. 定义标准拉普拉斯卷积核 [[0, 1, 0], [1, -4, 1], [0, 1, 0]]
    kernel = torch.tensor([[0, 1, 0], [1, -4, 1], [0, 1, 0]], dtype=torch.float32, device=device)
    # 将卷积核形状调整为 (out_channels, in_channels/groups, H, W) 
    # 这里我们对 RGB 三个通道分别计算，所以使用 groups=3
    kernel = kernel.view(1, 1, 3, 3).repeat(3, 1, 1, 1)
    
    # 2. 计算前景图和混合图的拉普拉斯算子
    # 使用 padding=1 保持图像尺寸不变
    lap_fg = F.conv2d(foreground_img, kernel, padding=1, groups=3)
    lap_blended = F.conv2d(blended_img, kernel, padding=1, groups=3)
    
    # 3. 提取掩码区域内的拉普拉斯值
    # 由于存在位移 (dx, dy)，我们需要通过掩码索引将源区域的纹理与目标区域对应起来
    fg_mask_bool = foreground_mask.expand(-1, 3, -1, -1).bool()
    bg_mask_bool = background_mask.expand(-1, 3, -1, -1).bool()
    
    # 提取出的值是 1D 向量
    source_lap_values = lap_fg[fg_mask_bool]
    blended_lap_values = lap_blended[bg_mask_bool]
    
    # 4. 计算均方误差 (MSE) 损失
    # 泊松编辑的目标是让混合区域的 Laplacian 等于源区域的 Laplacian
    loss = F.mse_loss(blended_lap_values, source_lap_values, reduction='sum')
